...
2021.02.10   
어제 만든 war파일에 오류가 생겨서 오류 수정하고 정상 작동 확인   
IE 버전별로 확인해 봤지만 지원하는 모든 버전에서 깨짐   
이벤트 구현 시 현재 작성된 버튼 클릭 함수를 기능에 맞게 분리해야 함   
테이블 클릭 함수는 테이블 선택된 회원의 key값을 하나 가져와서 그 key값으로 해당 회원의 상담 내역을 표시 할 수 있도록 해야 함   
- 다음에 할 일   
    이벤트 구현

2021.02.15   
클릭 시 작동은 임시로 kendo의 notification을 이용해 메세지만 출력 하도록 만들려고 했으나 실패   
onclick함수로 안되는 듯 한데 임시로 다른 방법을 사용한 버튼을 만들어 테스트 해봐야 할 것 같다   
- 다음에 할 일   
    이벤트 구현   
    
2021.02.16   
notification을 이용한 이벤트 넣기 완료   
```<span id="popupNotification"></span>```를 섹션(기능 할 컴포넌트가 있는 곳) 안에 추가해야 작동함   
- 해야 할 일   
    검색 기능   
    초기화 기능   
    고객 선택에 따른 상담내역 조회 기능   
    

2021.02.17   
이전에 만든 이벤트들을 용도에 따라 분류함   
submit, clear, save 버튼들은 단순 텍스트 출력, grid 안의 내용 선택 시 그 내용 출력   
- 다음에 할 일   
    submit버튼을 누르면 검색창에 입력된 값을 읽어들일 수 있도록 하기   
